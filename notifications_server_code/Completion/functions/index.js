...
exports.sendCompletion = functions.database.ref('/completions/{receiver_user_id}/{notification_id}')
.onWrite((data, context) =>
{
	...
	const task_title = admin.database().ref(`/completions/${receiver_user_id}/${notification_id}/taskTitle`).once('value');
	...
	return task_title.then(fromTaskResult => 

		{
			...
			console.log('The name of the task is : ', task_title);
			 	...
			 	const sender_user_id = admin.database().ref(`/completions/${receiver_user_id}/${notification_id}`).once('value');
						 	...
						 	return DeviceToken.then(result =>
						 	 {
						 	 	...
						 	 	const payload =
						 	 	{
						 	 		notification:
						 	 		{
						 	 			...
						 	 			body: `${senderUserName} Just Completed A Task (${from_task_title}).`,
						 	 			icon: "default"
						 	 		}
						 	 	};
